...
namespace Maple.ImGui.Backends.ImGuiCore
{
    public abstract class ImGuiBackendImpBase : IDisposable
    {
        ...
        public ImGuiBackendImpBase(ImGuiBackendBridgeCollection bridgeCollection, IImGuiUIView view)
        {
            ...
            this.View.TryDrawLauncher = TryDrawLauncher;
        }

        public virtual bool Initialize(nint context) => true;
        ...
        protected unsafe bool TryDrawLauncher()
        {
            var uintyInputBridge = this.BridgeCollection.UnityInputBridge;
            if (uintyInputBridge is null)
            {
            ...
            if (false == uintyInputBridge.TryDrawLauncher(out var nativePtr, out var u0, out var v0, out var u1, out var v1))
            {
                return false;
            }
            var textureId = GetOrCreateTextureId(nativePtr);
